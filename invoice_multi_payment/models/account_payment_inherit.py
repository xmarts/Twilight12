...
class account_payment(models.Model):
    ...
    complete_total = fields.Boolean(string='Complete', store=True)
    proyecto = fields.Char(string='Project', compute='_compute_project', store=True)
    ...
    @api.depends('partner_id','invoice_lines')
    def _compute_project(self):
        for record in self:
            x=""
            if len(record.invoice_lines) >0:
                for rec in record.invoice_lines:
                    x+= str(rec.proyecto)+" "
                    
            else:
                for rec in record.invoice_ids:
                    x= rec.project_id_from_so.name
                    if x == False:
                        x=" "
                    break
            record[("proyecto")]=str(x)
    ...
    @api.onchange('amount','invoice_lines')
    def onchnage_amount(self):
        total = 0.0
        remain = self.amount
        for line in self.invoice_lines:
            if line.complete_total == True:
...
class PaymentInvoiceLine(models.Model):
    ...
    proyecto = fields.Char(related="invoice_id.project_id_from_so.name", string="related")
